...
def transform_stim_trials_ctxtgt(tt, stm, dyns, ctx, gt, choice, eff_choice,
                                 rw, obsc, *kwargs):
    # deal with contexts
    ctxseq = []
    ...
    cohvalues = [0, 0, 0, 0, 0]
    cohvalues = np.sort(cohvalues)

    return stim_trials, cohvalues, ctxt_trials


def req_quantities_ctxtgt(stim_trials, cohvalues, stm, dyns, gt, choice,
                          eff_choice, rw, obsc):
    # ---- prepare LDA data
    Xdata, ydata = [], []
    Xdata_idx = []
    ...
    return Xdata, ydata, Xdata_idx, Xdata_resp, Xconds_2, Xacts_1,\
        Xrws_1, Xlfs_1, Xrse_6, rses, Xacts_0, Xrws_0, Xgts_0, Xcohs_0,\
        Xdc_idx_0, Xctxt_1, Xce_OH
    # ---- prepare LDA data
    Xdata, ydata = [], []
    Xdata_idx = []
    Xdc_idx_0 = []
        ...
        if (prev2_stats['rw'] == 1 and prev3_stats['rw'] == 1 and
           prev4_stats['rw'] == 1):
            if((prev4_stats['choice'] == -1 and prev3_stats['choice'] == -1 and
                prev2_stats['choice'] == -1) or (prev4_stats['choice'] == 1 and
                                                 prev3_stats['choice'] == 1 and
                                                 prev2_stats['choice'] == 1)):
                Xdc_idx_0 = np.append(Xdc_idx_0, curr_stats['start_end'][1]-1)
                Xdata_idx = np.append(Xdata_idx, curr_stats['start_end'][0]-1)
                # extract the block-level context information
                Xctxt_1 = np.append(Xctxt_1, prev1_stats['ctx'])
...
def sep_correct_error_ctxtgt(stm, dyns, Xdata, ydata, Xdata_idx, Xdata_resp,
                             Xctxt_1, Xce_OH, Xacts_1, Xrws_1, Xlfs_1, Xrse_6,
                             rses, Xacts_0, Xrws_0, Xgts_0, Xcohs_0, Xdc_idx_0,
                             margin=[1, 2], idd=1):
    # margin=[1,2]
    ...
    return Xdata_correct, correct_trial, Xdata_stim_correct, Xdata_dc_correct,\
        ...
        Xcohs_0_error, ydata_bias_correct, ydata_bias_error, ydata_conds_correct,\
        ...
        ydata_cgts_error, ydata_crws_correct, ydata_crws_error,\
