...
def bootstrap_linsvm_step(Xdata_hist_set,NN, ylabels_hist_set,unique_states,unique_cohs,files,false_files, pop_correct, pop_zero, pop_error, USE_POP, type, DOREVERSE=0, CONTROL = 0, STIM_PERIOD=0, n_iterations=10, N_pseudo_dec=25, ACE_RATIO=0.5, train_percent=0.6, RECORD_TRIALS=0, RECORDED_TRIALS_SET=[], mmodel=[],PCA_n_components=0):
    ...
    if USE_POP ==1:
        if CONTROL==1:
            NN = len(pop_zero)#+len(pop_error)
        elif CONTROL==2:
            NN = len(pop_zero)#+len(pop_correct)
        elif CONTROL==0:
            NN = len(pop_zero)#+len(pop_correct)+len(pop_error)
...
def shuffle_linsvm_proj_step(coeffs_pool, intercepts_pool, Xdata_hist_set,NN, ylabels_hist_set,unique_states,unique_cohs,files,false_files, pop_correct, pop_zero, pop_error, USE_POP, type, DOREVERSE=0, CONTROL=1, n_iterations=10, N_pseudo_dec=25, train_percent=0.6, RECORD_TRIALS=0, RECORDED_TRIALS_SET=[],mmodel=[],PCA_n_components=0):
    nlabels = 6*(len(files)-len(false_files))
    ntrain  = int(train_percent*N_pseudo_dec)
    ntest   = (N_pseudo_dec-ntrain)*4 # state
    ...
    for i in range(n_iterations):
        ...
        if USE_POP==1:
            if CONTROL==1:
                pop_ez = pop_zero #np.union1d(pop_error,pop_zero)
                Xdata_testc, Xdata_teste = Xdata_testc[:,pop_ez],Xdata_teste[:,pop_ez] 
            elif CONTROL==2:
                pop_cz = pop_zero#np.union1d(pop_correct,pop_zero)
                Xdata_testc, Xdata_teste = Xdata_testc[:,pop_cz],Xdata_teste[:,pop_cz]
            elif CONTROL==0:
                pop_ce = pop_zero#np.union1d(pop_correct,pop_zero)
                Xdata_testc, Xdata_teste = Xdata_testc[:,pop_ce],Xdata_teste[:,pop_ce]
...
def bootstrap_linsvm_step_fixationperiod_balanced(Xdata_set,NN, ylabels_set,unique_states,unique_cohs,files,false_files, coh_ch_stateratio_correct,coh_ch_stateratio_error, pop_correct,pop_error, USE_POP, type, DOREVERSE=0, CONTROL = 0, STIM_PERIOD=0, n_iterations=10, N_pseudo_dec=5, ACE_RATIO=0.5, train_percent=0.6, RECORD_TRIALS=0, RECORDED_TRIALS_SET=[],mmodel=[],PCA_n_components=0):
    ### ac/ae ratio 
    CRATIO = 0.5#ACE_RATIO/(1+ACE_RATIO)
    ERATIO = 1-CRATIO
    #  
    ...
    for i in range(n_iterations):
        if (i+1) % PRINT_PER == 0:
            print(i)
        Xmerge_trials_correct,ymerge_labels_correct,Xmerge_trials_error,ymerge_labels_error,merge_trials=gpt.merge_pseudo_beh_trials_balanced(Xdata_set,ylabels_set,unique_states,COHAXISS,files, false_files, coh_ch_stateratio_correct,coh_ch_stateratio_error,EACHSTATES=N_pseudo_dec, RECORD_TRIALS=RECORD_TRIALS, RECORDED_TRIALS_SET=RECORDED_TRIALS_SET[i],STIM_BEH=1)#### from unique_cohs to COHAXISS
